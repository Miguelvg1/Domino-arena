...
// Top 10 semanal oficial (mínimo 10 PJ), Jugador del Día y cola global sin mesa.
...
async function playerOfDay(){
 ...
 busyDay=true;
 try{
  const now=new Date(),r=await sb.rpc('titanes_ranking_v2_compat',{p_from:localDayStart().toISOString(),p_to:now.toISOString(),p_min_games:0});
  if(r.error)throw r.error;
  const x=(r.data||[])[0];
  if(!x)return;
  const card=document.createElement('section');card.className='playerDayCard';
  card.innerHTML=`<div class="playerDayCrown">👑</div><div class="playerDayMain"><small>JUGADOR DEL DÍA</small><h2>${esc(x.player_name)}</h2><div class="playerDayStats"><span><b>${x.games_won||0}</b>Ganadas</span><span><b>${x.games_lost||0}</b>Perdidas</span><span><b>${x.games_played||0}</b>Partidas</span><span><b>${Number(x.win_pct||0).toFixed(1)}%</b>Average</span><span><b>${Number(x.total_points||0).toLocaleString()}</b>Puntos</span></div></div>`;
