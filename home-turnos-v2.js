...
async function fixTop10(){
 ...
 if(!head||busyTop)return;
 const panel=head.parentElement;if(panel.querySelectorAll('.officialWeeklyRow').length>0&&panel.querySelectorAll('.officialWeeklyRow').length<=10&&panel.dataset.officialTop==='1')return;
 busyTop=true;
 try{
  ...
  panel.innerHTML=`<div class="row"><h2 class="grow">TOP 10 SEMANAL</h2><span class="statPill">máx. 10 · mín. 10 PJ</span></div>${rows.map((x,i)=>`<div class="rivalListItem officialWeeklyRow ${i<3?'top'+(i+1):''}"><b style="width:24px">${i+1}</b><div class="grow"><b>${esc(x.player_name)}</b></div><strong>${Number(x.win_pct||0).toFixed(1)}%</strong><span class="small muted">${x.games_played||0} PJ</span></div>`).join('')||'<p class="muted">Todavía no hay jugadores con 10 partidas esta semana.</p>'}`;
 }catch(e){console.warn('Top 10 oficial',e)}finally{busyTop=false}
}
async function top5Day(){
 if(busyDayTop)return;
 const weeklyHead=[...document.querySelectorAll('h2')].find(x=>x.textContent.trim().toUpperCase()==='TOP 10 SEMANAL');
 if(!weeklyHead)return;
 const weeklyPanel=weeklyHead.parentElement;
...
const mo=new MutationObserver(()=>{clearTimeout(window.__htv2);window.__htv2=setTimeout(apply,100)});mo.observe(document.documentElement,{childList:true,subtree:true});
